...
We are aware that scSHC does not run on Windows machine at the moment due to the mccores error. If you are on a Windows machine, specify this as a parameter as ```windows = TRUE``` in the function ```get_clust_assignments()```. 
...
ari_table <- data.frame("Algorithm" = c("CHAI_CSPA", "CHAI_SNF"), "ARI" = c(eval_function(cspa_clusters@.Data, sdf_metadata$Graph.based),eval_function(snf_clusters@.Data, sdf_metadata$Graph.based)))
